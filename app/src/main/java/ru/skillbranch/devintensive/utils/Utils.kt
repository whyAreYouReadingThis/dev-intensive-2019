...
object Utils {
    ...
    private val mapForTransliteration = mapOf(
    ...
    fun transliteration(payload: String, divider: String = " ") = buildString {
        payload.forEach {
            append(
                when {
                    it == ' ' -> divider
                    it.isUpperCase() -> mapForTransliteration[it.toLowerCase()]?.capitalize() ?: it.toString()
                    else -> mapForTransliteration[it] ?: it.toString()
                }
            )
        }
    }
